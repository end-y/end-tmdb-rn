...
import { get_movie_by_id, get_tvSeries_by_id } from "../api"
...
const Movie = () => {
    ...
    useEffect(() => {
        async function get(){
            let res = await get_tvSeries_by_id((id as string))
            setMovie(res)
        }
        get()
    },[])
         ...
         <View style={styles.main} >
            ...
            <View style={styles.infMain}>
                <View style={{maxWidth:"75%"}}>
                    <Text style={styles.head}>{movie.original_name}</Text>
                    <Text style={{fontWeight:"500"}}>{new Date(movie.first_air_date).toLocaleDateString()}</Text>
                </View>
